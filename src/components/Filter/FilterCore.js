                  ...
                  <div class="input-group-btn">
                    ...
                    <div class="btn-group" id="_btnValue{{$key}}" ng-show="!$value.query.label">
                      <button type="button" class="btn btn-default" ng-click="toggleUpdatingValue(this, $key)" ng-disabled="validatonValue($value)" id="_valueLabel{{$key}}">
                          <span ng-if="$value.query.attribute.type != 'select'" id="_conditionLabel{{$key}}">{{ $value.query.value ? $value.query.value.push && $value.query.value.length > 0  ?  $value.query.value.join(', ') : $value.query.value : ('gumga.filter.value' | translate) | gumgaGenericFilter:$value.query.attribute.type}} </span>
                          <span ng-if="$value.query.attribute.type === 'select'" id="_conditionLabel{{$key}}">{{ $value.query.value.length > 0 ? ('gumga.filter.' + $value.query.value | translate) : '' }}</span>
                      </button>
